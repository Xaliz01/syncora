...
import type { TrialTestDataStatusResponse } from "@planwise/shared";
 ...
 * Deux scénarios (sans / avec injection démo), alignés sur signup-onboarding.live.spec.ts.
 */

async function signupThroughOnboardingDemoStep(page: Page): Promise<void> {
  const email = `nouveau.${Date.now()}@exemple.fr`;
  ...
  let demoStatus: TrialTestDataStatusResponse = {
    status: "none",
    hasTestData: false,
    injectedAt: null,
  };
  let demoStatusPollsWhileInjecting = 0;
  ...
  const prefs = () => ({
    ...
    preferences: {
      ...
      setupGuideDismissedOrganizationIds: setupGuideDismissed ? [foundingAdmin.organizationId] : [],
      setupGuideDismissed,
    },
  });
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (method === "POST" && path.includes("/trial-test-data/inject")) {
      demoStatus = { status: "injecting", hasTestData: false, injectedAt: null };
      demoStatusPollsWhileInjecting = 0;
      await route.fulfill({ json: { accepted: true } });
      return;
    }
    if (method === "GET" && path.includes("/trial-test-data/status")) {
      if (demoStatus.status === "injecting") {
        demoStatusPollsWhileInjecting += 1;
        if (demoStatusPollsWhileInjecting >= 2) {
          demoStatus = {
            status: "ready",
            hasTestData: true,
      ...
      await route.fulfill({ json: demoStatus });
      return;
    }
