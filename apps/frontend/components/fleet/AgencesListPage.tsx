...
export function AgencesListPage() {
  ...
  return (
    <ListPageRoot>
      ...
      <ListToolbar>
        <ListSearchField value={search} onChange={setSearch} placeholder="Filtrer par nom, ville, code postal…" />
      </ListToolbar>
      ...
      {loading ? (
        ...
        <ListEmptyState
          ...
          action={
            <Link href="/fleet/agences/new" className="text-sm text-brand-600 dark:text-brand-400 hover:underline font-medium">
        ...
        >
          {filtered.map((agence) => (
            <ListRowLink key={agence.id} href={`/fleet/agences/${agence.id}`} gridTemplateClass={GRID}>
