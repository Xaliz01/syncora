...
export function CaseCustomerPicker({
  ...
  return (
    <div className="space-y-2">
      <label htmlFor={`${idPrefix}-trigger`} className="block text-sm font-medium text-slate-700">
        Client
      </label>
      ...
      <div className="relative">
        <button
          ...
          className="flex w-full items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-left text-sm text-slate-800 hover:border-slate-300 disabled:opacity-50"
        >
          ...
          <div className="absolute z-40 mt-1 w-full rounded-lg border border-slate-200 bg-white shadow-lg">
            {!showCreate ? (
              <div className="p-2">
                ...
                />
                {debounced.length === 1 && (
                  <p className="mt-2 px-1 text-xs text-slate-500">Saisissez au moins 2 caractères pour filtrer.</p>
                )}
                <div className="mt-2 max-h-48 overflow-y-auto">
                  {isFetching && <div className="px-2 py-2 text-xs text-slate-500">Chargement…</div>}
                  {!isFetching &&
                    list.map((c) => (
                      <button
                        key={c.id}
                        ...
                        className="flex w-full flex-col items-start rounded-md px-2 py-2 text-left text-sm hover:bg-slate-50"
                      >
                  ...
                  {!isFetching && list.length === 0 && debounced.length !== 1 && (
                    <p className="px-2 py-2 text-xs text-slate-500">Aucun client trouvé.</p>
                  )}
                </div>
                <div className="mt-2 flex flex-wrap gap-2 border-t border-slate-100 pt-2">
                  ...
                  <button
                    ...
                    className="ml-auto rounded-md px-2.5 py-1.5 text-xs text-slate-500 hover:bg-slate-50"
                  >
                    Fermer
                  </button>
                </div>
