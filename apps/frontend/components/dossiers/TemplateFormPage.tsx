...
export function TemplateFormPage({ templateId }: { templateId?: string }) {
  ...
  const { data: existing } = useQuery({
    queryKey: ["dossier-template", templateId],
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["dossier-templates"] });
      router.push("/dossiers/templates");
    },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["dossier-templates"] });
      queryClient.invalidateQueries({ queryKey: ["dossier-template", templateId] });
      router.push("/dossiers/templates");
    },
    onError: (err: Error) => setError(err.message)
  });
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3">
          ...
          <button
            type="button"
            onClick={() => router.push("/dossiers/templates")}
