...
const STATUS_TRANSITIONS: Record<DossierStatus, DossierStatus[]> = {
...
export function DossierDetailPage({ dossierId }: { dossierId: string }) {
  ...
  const { data: interventions } = useQuery({
    queryKey: ["interventions", dossierId],
    queryFn: () => api.listInterventions({ dossierId })
  });
  ...
  const [editPriority, setEditPriority] = useState<DossierPriority>("medium");
  const [editAssignee, setEditAssignee] = useState("");
  const [editDueDate, setEditDueDate] = useState("");

  const invalidateAll = () => {
    queryClient.invalidateQueries({ queryKey: ["dossier", dossierId] });
    queryClient.invalidateQueries({ queryKey: ["interventions", dossierId] });
    queryClient.invalidateQueries({ queryKey: ["dossiers"] });
  };

  const statusMutation = useMutation({
    mutationFn: (status: DossierStatus) => api.updateDossier(dossierId, { status }),
    onSuccess: invalidateAll
  });

  const updateMutation = useMutation({
    mutationFn: (payload: api.UpdateDossierPayload) => api.updateDossier(dossierId, payload),
    onSuccess: () => {
      invalidateAll();
      setIsEditing(false);
    }
  ...
  const deleteMutation = useMutation({
    mutationFn: () => api.deleteDossier(dossierId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["dossiers"] });
  ...
  const todoMutation = useMutation({
    mutationFn: (params: { stepId: string; todoId: string; status: TodoItemStatus }) =>
      api.updateTodo(dossierId, params),
    onSuccess: invalidateAll
  });
  ...
  const allowedTransitions = STATUS_TRANSITIONS[dossier.status] ?? [];
  const isOverdue = dossier.dueDate && new Date(dossier.dueDate) < new Date() && dossier.status !== "completed" && dossier.status !== "cancelled";

  const startEditing = () => {
    setEditTitle(dossier.title);
    setEditDesc(dossier.description ?? "");
    setEditPriority(dossier.priority);
    setEditAssignee(dossier.assigneeId ?? "");
    setEditDueDate(dossier.dueDate ? dossier.dueDate.split("T")[0] : "");
    setIsEditing(true);
  };
    ...
    <div className="space-y-6">
          ...
          {isEditing ? (
            <div className="space-y-3 max-w-xl">
              ...
              <div className="grid grid-cols-3 gap-3">
                <select
                  value={editPriority}
                  onChange={(e) => setEditPriority(e.target.value as DossierPriority)}
            ...
            <>
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className={`rounded-full border px-2.5 py-0.5 text-xs font-medium ${STATUS_COLORS[dossier.status]}`}>
                  {STATUS_LABELS[dossier.status]}
                </span>
                <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${
                  dossier.priority === "urgent" ? "bg-red-50 text-red-600" :
                  dossier.priority === "high" ? "bg-orange-50 text-orange-600" :
                  dossier.priority === "medium" ? "bg-blue-50 text-blue-600" :
              ...
              {dossier.description && (
                <p className="mt-1 text-sm text-slate-500">{dossier.description}</p>
              )}
              <div className="mt-2 flex items-center gap-4 text-xs text-slate-500">
                {dossier.assigneeName && <span>Assigné à : {dossier.assigneeName}</span>}
                {dossier.dueDate && (
                  <span>Échéance : {new Date(dossier.dueDate).toLocaleDateString("fr-FR")}</span>
                )}
      ...
      <div className="rounded-xl border border-blue-100 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-sm font-semibold text-slate-800">{dossier.progress}%</div>
        </div>
        <div className="w-full h-2 rounded-full bg-slate-100">
          <div
            className={`h-full rounded-full transition-all ${
              dossier.progress === 100 ? "bg-green-500" : "bg-blue-500"
            }`}
      ...
      {dossier.steps.length > 0 && (
        <div className="space-y-4">
          ...
          {[...dossier.steps].sort((a, b) => a.order - b.order).map((step) => {
