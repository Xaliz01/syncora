...
import { LANDING_ACCOMPANIMENT } from "@/lib/landing-copy";
 ...
 */
export function AccompanimentSupportBlock({ variant = "landing" }: { variant?: Variant }) {
  ...
  return (
    <aside
      className={
        banner
          ? "rounded-2xl border-2 border-brand-300 dark:border-brand-500/50 bg-white/95 dark:bg-slate-900/95 px-5 py-4 sm:px-6 sm:py-5 shadow-md shadow-brand-600/10 ring-1 ring-brand-100 dark:ring-brand-900/40"
    ...
    >
      ...
      >
        <div className={banner ? "min-w-0 lg:max-w-sm shrink-0" : undefined}>
          {!banner ? (
            <span className="inline-flex items-center rounded-full bg-brand-600 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white mb-3">
              Accompagnement
            </span>
          ) : (
            <span className="inline-flex items-center rounded-full bg-brand-600 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white mb-2">
              Accompagnement
            </span>
          )}
          <h2
            ...
            <div className="mt-4 flex flex-col items-start gap-2">
              ...
              {chatReady ? (
                <button
                  type="button"
                  onClick={onOpenChat}
                  className="inline-flex rounded-lg border border-brand-200 dark:border-brand-500/40 bg-brand-50 dark:bg-brand-950/40 px-3.5 py-2 text-sm font-medium text-brand-700 dark:text-brand-300 hover:bg-brand-100 dark:hover:bg-brand-950/70 transition"
                >
                  Ouvrir le chat
                </button>
              ) : (
        ...
        <ul className={banner ? "grid gap-3 sm:grid-cols-3 flex-1 min-w-0" : "mt-5 space-y-3"}>
          {LANDING_ACCOMPANIMENT.points.map((point) => (
            <li
              key={point.title}
              className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-950/50 px-3.5 py-3"
            >
