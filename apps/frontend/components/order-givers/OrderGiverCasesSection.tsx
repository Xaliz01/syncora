...
export function OrderGiverCasesSection({ orderGiverId }: { orderGiverId: string }) {
  const [offset, setOffset] = useState(0);

  const { data, isLoading } = useQuery({
    queryKey: ["cases", "order-giver", orderGiverId, offset],
    queryFn: () =>
      casesApi.listCases({
        orderGiverId,
    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 shadow-sm dark:shadow-slate-950/20">
      ...
      {!isLoading && total === 0 && (
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Aucun dossier lié à ce donneur d&apos;ordre.
        </p>
      )}
