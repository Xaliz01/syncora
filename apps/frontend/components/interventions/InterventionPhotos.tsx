...
interface Props {
  ...
  /** Compact mode for MyDayPage cards (thumbnails + upload actions) */
  compact?: boolean;
  readOnly?: boolean;
  /** When false, upload buttons are hidden (e.g. camera triggered from parent action row) */
  showUploadActions?: boolean;
}

export const InterventionPhotos = forwardRef<InterventionPhotosHandle, Props>(
  function InterventionPhotos(
    { interventionId, compact = false, readOnly = false, showUploadActions = true },
    ref,
  ) {
    ...
    const showCompactUpload = !readOnly && showUploadActions;
    const showGalleryOnly = !readOnly && !showUploadActions;

    if (compact) {
      if (readOnly && !loading && photos.length === 0) {
        return null;
      }
      ...
      return (
        <div className="mt-3 border-t border-slate-200/80 dark:border-slate-700/80 pt-3">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs font-medium text-slate-600 dark:text-slate-300">Photos terrain</p>
            {photos.length > 0 && (
          ...
          {loading ? (
            <div className="flex gap-2">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-14 w-14 rounded-lg bg-slate-100 dark:bg-slate-800 animate-pulse"
                />
              ))}
            </div>
          ) : photos.length > 0 ? (
            ...
            <button
              ...
              disabled={uploading}
              className="w-full rounded-lg border border-dashed border-slate-300 dark:border-slate-600 bg-white/70 dark:bg-slate-800/50 px-3 py-4 text-xs text-slate-500 dark:text-slate-400 hover:border-brand-400 hover:text-brand-600 dark:hover:text-brand-400 transition disabled:opacity-50"
            >
              {uploading ? "Envoi en cours…" : "Aucune photo — appuyez pour en ajouter"}
            </button>
          ) : null}
          ...
          {showGalleryOnly && (
            <button
              ...
              onClick={() => fileInputRef.current?.click()}
              disabled={uploading}
              className="mt-2 text-xs font-medium text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 disabled:opacity-50"
            >
              {uploading ? "Envoi en cours…" : "Importer depuis la galerie"}
            </button>
          )}
