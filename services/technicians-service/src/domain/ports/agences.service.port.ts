...
export abstract class AbstractAgencesService {
  ...
  abstract getAgence(organizationId: string, agenceId: string): Promise<AgenceResponse>;
  abstract listAgences(organizationId: string): Promise<AgenceResponse[]>;
  abstract deleteAgence(organizationId: string, agenceId: string): Promise<{ deleted: true }>;
}
