...
@Injectable()
export class PennylaneBillingAdapter implements BillingProviderAdapter {
  readonly provider: IntegrationProvider = PENNYLANE_PROVIDER;
  ...
  async getStatus(organizationId: string): Promise<PennylaneConnectionStatus> {
    const orgId = requireOrganizationId(organizationId);
    const oauthAvailable = this.isOAuthConfigured();
    const doc = await this.credentialModel
      .findOne({ ...organizationScopeFilter(orgId), provider: PENNYLANE_PROVIDER })
      .exec();
    if (!doc) {
      return { provider: PENNYLANE_PROVIDER, connected: false, oauthAvailable };
    }
    return {
  ...
  async startOAuth(organizationId: string): Promise<PennylaneOAuthStartResponse> {
    const orgId = requireOrganizationId(organizationId);
  ...
  async completeOAuth(body: CompletePennylaneOAuthBody): Promise<void> {
    ...
    await this.persistOAuthTokens(orgId, tokens);
  }

  async connect(body: ConnectPennylaneBody): Promise<void> {
    const orgId = requireOrganizationId(body.organizationId);
    ...
    await this.credentialModel
      .findOneAndUpdate(
        { organizationId: orgId, provider: PENNYLANE_PROVIDER },
        {
  ...
  async disconnect(organizationId: string): Promise<PennylaneConnectionStatus> {
    const orgId = requireOrganizationId(organizationId);
    const doc = await this.credentialModel
      .findOne({ ...organizationScopeFilter(orgId), provider: PENNYLANE_PROVIDER })
      .exec();
    if (doc) {
      await this.beforeDisconnect(doc);
    }
    await this.credentialModel
      .deleteOne({ ...organizationScopeFilter(orgId), provider: PENNYLANE_PROVIDER })
      .exec();
    return {
      provider: PENNYLANE_PROVIDER,
      connected: false,
  ...
  async beforeDisconnect(doc: IntegrationCredentialDocument): Promise<void> {
    if (doc.authMethod !== "oauth") return;
    await this.revokeOAuthTokenBestEffort(doc);
  }

  async syncCase(body: SyncCaseToPennylaneBody): Promise<SyncCaseToPennylaneResult> {
    const orgId = requireOrganizationId(body.organizationId);
    if (!body.caseId?.trim()) {
  ...
  async deleteRemoteDraft(doc: IntegrationSyncDocument): Promise<void> {
    const token = await this.requireAccessToken(doc.organizationId);
    await this.pennylaneDelete(token, `/customer_invoices/${syncRemoteInvoiceId(doc)}`);
  }
  ...
  private async requireAccessToken(organizationId: string): Promise<string> {
    const doc = await this.credentialModel
      .findOne({ ...organizationScopeFilter(organizationId), provider: PENNYLANE_PROVIDER })
      .exec();
    if (!doc) {
  ...
  private async pennylaneDelete(token: string, path: string): Promise<void> {
    try {
      await firstValueFrom(
        this.httpService.delete(`${this.apiBase}${path}`, {
          headers: {
            Authorization: `Bearer ${token}`,
            Accept: "application/json",
          },
