...
import { AbstractArticleStockService } from "./ports/article-stock.service.port";
...
import {
  ...
  ensureStrictlyPositiveNumber,
  isDuplicateKeyError,
} from "./utils/validation.utils";

@Injectable()
export class ArticleStockService extends AbstractArticleStockService {
  constructor(
    @InjectModel("Article")
  ...
  async createArticle(body: CreateArticleBody): Promise<ArticleResponse> {
    ...
    const initialStock = ensureNonNegativeNumber(body.initialStock ?? 0, "initialStock");
    const reorderPoint = ensureNonNegativeNumber(body.reorderPoint ?? 0, "reorderPoint");
    ...
    const defaultPrice =
      body.defaultPrice !== undefined && body.defaultPrice !== null
        ? ensureNonNegativeNumber(body.defaultPrice, "defaultPrice")
        : undefined;

    const locationId = body.locationId
      ? await this.stockLocationService.resolveLocationId(body.organizationId, body.locationId)
      : await this.stockLocationService.getDefaultLocationId(body.organizationId);
    ...
    try {
      ...
      if (initialStock > 0) {
        const locationName = locationId
          ? await this.stockLocationService.getLocationName(body.organizationId, locationId)
          : undefined;
  ...
  async listArticles(
    ...
    return { articles: docs.map((doc) => toArticleResponse(doc)), total };
  }
  ...
  async updateArticle(id: string, body: UpdateArticleBody): Promise<ArticleResponse> {
    ...
    if (body.reorderPoint !== undefined) {
      doc.reorderPoint = ensureNonNegativeNumber(body.reorderPoint, "reorderPoint");
    }
    if (body.targetStock !== undefined) {
      doc.targetStock = ensureNonNegativeNumber(body.targetStock, "targetStock");
    }
  ...
  async createArticleMovement(body: CreateArticleMovementBody): Promise<StockMovementResponse> {
    ...
    const quantity =
      movementType === "adjustment"
        ? ensureNonNegativeNumber(body.quantity, "quantity")
        : ensureStrictlyPositiveNumber(body.quantity, "quantity");
  ...
  async addInterventionArticleUsage(
    ...
    const quantity = ensureStrictlyPositiveNumber(body.quantity, "quantity");
  ...
  async listArticleMovements(
    ...
    const docs = await this.stockMovementModel
      ...
      .limit(limit)
      .exec();
    return docs.map((doc) => toStockMovementResponse(doc));
  }
  ...
  async createStockTransfer(body: CreateStockTransferBody): Promise<StockMovementResponse> {
    ...
    await this.stockLocationService.resolveLocationId(body.organizationId, body.sourceLocationId);
    await this.stockLocationService.resolveLocationId(
      body.organizationId,
    ...
    const sourceName = await this.stockLocationService.getLocationName(
      body.organizationId,
      body.sourceLocationId,
    );
    const destName = await this.stockLocationService.getLocationName(
      body.organizationId,
      body.destinationLocationId,
    );
    ...
    return toStockMovementResponse(movementDoc);
  }
  ...
  private async applyStockMovement(params: {
    ...
    const locationName = params.locationId
      ? await this.stockLocationService.getLocationName(params.organizationId, params.locationId)
      : undefined;
    ...
    return {
      movement: toStockMovementResponse(movementDoc),
    };
  }
