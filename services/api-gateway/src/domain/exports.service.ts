...
export class ExportsGatewayService extends AbstractExportsGatewayService {
  ...
  async exportCaseSummaryPdf(user: AuthUser, caseId: string): Promise<ExportResult> {
    const caseData = await this.callService<CaseResponse>(CASES_URL, user.organizationId, {
      method: "get",
      path: `/cases/${caseId}`,
    });
    ...
    if (caseData.customerId) {
      try {
        customer = await this.callService<CustomerResponse>(CUSTOMERS_URL, user.organizationId, {
          method: "get",
          path: `/customers/${caseData.customerId}`,
        });
      } catch {
    ...
    const interventions = await this.callService<InterventionResponse[]>(
      CASES_URL,
      user.organizationId,
      { method: "get", path: "/interventions", query: { caseId } },
    );
  ...
  async exportCasesList(
    ...
    const cases = await this.callService<CaseSummaryResponse[]>(CASES_URL, user.organizationId, {
      method: "get",
      path: "/cases",
      query,
    });
  ...
  async exportUsersList(user: AuthUser, format: ExportFormat): Promise<ExportResult> {
    const users = await this.callService<UserResponse[]>(USERS_URL, user.organizationId, {
      method: "get",
      path: "/users",
    });
  ...
  async exportCustomersList(
    ...
    const customers = await this.callService<CustomerResponse[]>(
      CUSTOMERS_URL,
      user.organizationId,
      { method: "get", path: "/customers", query },
    );
  ...
  async exportInterventionsList(
    ...
    let interventions = await this.callService<InterventionResponse[]>(
      CASES_URL,
      user.organizationId,
      { method: "get", path: "/interventions", query },
    );
  ...
  async exportMileageReport(
    ...
    const teams = await this.callService<TeamResponse[]>(TECHNICIANS_URL, user.organizationId, {
      method: "get",
      path: "/teams",
    });
  ...
  async exportDashboardTodoCases(
    ...
    const cases = await this.callService<DashboardTodoCaseItem[]>(CASES_URL, user.organizationId, {
      method: "get",
      path: "/cases/dashboard/todo-cases",
      query: { templateId: params.templateId, todoLabel: params.todoLabel },
    });
  ...
  async getReportingStats(user: AuthUser): Promise<ReportingStatsResponse> {
    const [cases, interventions, technicians, customers] = await Promise.all([
      this.callService<CaseSummaryResponse[]>(CASES_URL, user.organizationId, {
        method: "get",
        path: "/cases",
      }),
      this.callService<InterventionResponse[]>(CASES_URL, user.organizationId, {
        method: "get",
        path: "/interventions",
      }),
      this.callService<TechnicianResponse[]>(TECHNICIANS_URL, user.organizationId, {
        method: "get",
        path: "/technicians",
      }),
      this.callService<CustomerResponse[]>(CUSTOMERS_URL, user.organizationId, {
        method: "get",
        path: "/customers",
      }),
    ]);
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let rowIdx = 0; rowIdx < rows.length; rowIdx++) {
        ...
        row.forEach((cell, i) => {
          doc
            .fontSize(8)
            .text(cell, startX + i * colWidth + 4, rowY, { width: colWidth - 8, lineBreak: false });
        });
        doc.y = rowY + 16;
      }
  ...
  private callService<T>(
    ...
    params: { method: "get" | "post"; path: string; query?: Record<string, string>; body?: object },
  ): Promise<T> {
    return this.scopedHttp.request<T>({
      baseUrl,
      organizationId,
      errorLabel: "Export service error",
      method: params.method,
      path: params.path,
      body: params.body,
      query: params.query ? { ...params.query, organizationId } : { organizationId },
