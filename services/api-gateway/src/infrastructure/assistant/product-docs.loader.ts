...
Tableau de bord (/) ; Ma journée (/my-day) ; Dossiers (/cases) ; Nouveau dossier (/cases/new) ; Planning (/cases/calendar) ; Contrats (/contracts) ; Nouveau contrat (/contracts/new) ; Mouvements de stock (/stock) ; Reporting (/reporting) ; Facturation (/billing) ; Clients (/customers) ; Nouveau client (/customers/new) ; Donneurs d'ordre (/order-givers) ; Utilisateurs (/users) ; Inviter (/users/new) ; Équipes (/fleet/teams) ; Techniciens (/fleet/technicians) ; Véhicules (/fleet/vehicles) ; Agences (/fleet/agences) ; Catalogue articles (/settings/stock/articles) ; Prestations (/settings/prestations) ; Emplacements stock (/settings/stock/locations) ; Modèles de dossier (/settings/case-templates) ; Types d'intervention (/settings/intervention-types) ; Profils (/settings/profiles) ; Notifications (/settings/notifications) ; Intégrations (/settings/integrations) ; Import de données (/settings/data-import) ; Recherche (/search) ; Mon organisation (/organization) ; Mon abonnement (/subscription) ; Mon compte (/account).
...
Étapes : 1) Suivi → Dossiers (/cases) puis Nouveau, ou /cases/new. 2) Titre, client, donneur d'ordre si besoin. 3) Modèle de dossier (/settings/case-templates, import métiers possible). 4) Fiche : étapes, tâches, interventions, documents, devis, stock consommé.
