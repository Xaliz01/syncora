...
Fiches détail dynamiques (/cases/:id, /customers/:id…) : ne pas inventer d'ID — guider vers la liste puis ouvrir depuis la liste. Modification dossier : depuis la fiche, bouton Modifier → /cases/:id/edit (permission cases.update). Modification organisation : depuis Mon organisation, bouton Modifier → /organization/edit (permission organizations.update). Modification compte : depuis Mon compte, bouton Modifier → /account/edit.
...
Assignation : technicien ou équipe (pas utilisateur). Terrain du jour : /my-day.`,
  },
  {
    ...
    text: `Règle : les factures clients s'émettent dans Planwise. Devis (PDF, lignes, TVA) sur un dossier, puis Créer une facture (complète, situation, acompte, solde) et avoir si besoin. Une situation se saisit en avancement cumulé (ex. 70 %) : seule la part non encore facturée est émise (acomptes et situations précédents compris) ; 100 % clôture le devis comme un solde. Situation, acompte et solde exigent un devis accepté. On peut aussi facturer une intervention ou plusieurs interventions du même dossier : bouton Facturer sur la carte, ou cases à cocher puis Créer une facture ; les lignes reprennent les articles et prestations consommés (quantité nette / quantité), encore modifiables. Sur l'intervention, Ajouter articles / prestations enregistre aussi les prestations (sans emplacement stock). L'émission via la facturation électronique arrivera bien avant l'obligation prévue le 1er septembre 2027 pour les artisans, TPE et PME.
...
Prestations catalogue : /settings/prestations. Abonnement Planwise (/subscription) ≠ facturation clients.`,
  },
  {
    ...
    text: `La facturation clients se fait dans Planwise (dossier ou /billing). L'émission via la facturation électronique arrivera bien avant l'obligation prévue le 1er septembre 2027 pour les artisans, TPE et PME. Pas de menu Intégrations. Escalade Crisp si question sur la facturation électronique.`,
  },
  {
...
Documents déposés sur les fiches dossier. Ne jamais dire « aucune limite » ni inventer un nombre max de fichiers. Ne pas confondre avec le stock articles (pièces) : ici = fichiers/documents.`,
  ...
  },
  {
    ...
    text: `Mon compte /account : fiche lecture (identité, mot de passe, thème, sidebar, sessions, liens légaux). Modifier → /account/edit : nom et préférences d'affichage (formulaire pleine page). Thème aussi via toggle header.
